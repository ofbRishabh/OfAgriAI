...
const CreditScoringModel = () => {
  ...
  const handleScore = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      // Simple demo scoring
      const score =
        Object.values(inputs).reduce((a, b) => a + b, 0) * 10 +
  ...
  return (
    <div className="w-full max-w-lg mx-auto bg-white rounded-xl shadow p-4 border flex flex-col">
      <div className="font-bold text-purple-700 mb-2 flex items-center gap-2">
        <ShieldCheck className="h-4 w-4" /> Credit Scoring Model
      </div>
      <form onSubmit={handleScore} className="space-y-4 mt-2">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input
              type="number"
              name="cropHistory"
              value={inputs.cropHistory}
              onChange={handleChange}
              min={0}
              max={10}
              className="w-full border-2 border-purple-200 rounded-lg px-3 py-2 focus:outline-none focus:border-purple-400"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
        ...
        <button
          className="bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-lg font-semibold w-full transition"
          type="submit"
          disabled={loading}
        >
